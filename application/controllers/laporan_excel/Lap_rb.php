<?php
      ...
      class Lap_rb extends CI_Controller{
            ...
            public function index($date_from_laporan,$date_to_laporan,$barang_id,$kategori_id,$sales_id,$pelanggan_id,$supplier_id,$no_polisi_id,$anggota_id,$pemakai_id,$lokasi_id)
            {
              ...
              $total_day=intval(((round(abs(strtotime($date_from_laporan) - strtotime($date_to_laporan)) / (60*60*24),0))+1)/2);
                ...
                $month_int = intval(date_format($date,"m"));
                  ...
                  $spreadsheet->getActiveSheet()->mergeCells('A'.$row.':J'.$row);
                  $sheet = $spreadsheet->getActiveSheet();
                  $sheet->setCellValue('A'.$row, 'Laporan Retur Pembelian');
                  $sheet->getStyle('A'.$row)->getAlignment()->setHorizontal('center');
                  ...
                  $sheet->setCellValue('D'.$row, 'INV / INV Pembelian');
                  ...
                  $data_logic = 0;
                  $key=0;
                  $read_select = $this->m_t_t_t_retur_pembelian->select_range_date($date_from_laporan,$date_to_laporan,0);
                  ...
                  {   
                    $data_logic = 1;
                        ...
                        $r_inv[$key]=$value->INV.' / '.$value->INV_PEMBELIAN;
                  ...
                  for($i=0;$i<=$total_data;$i++)
                  {
                        ...
                        $read_select = $this->m_t_t_t_retur_pembelian_rincian->select($r_id[$i]);
                  ...
                  $row = $row + 1;
                            $sheet->setCellValue('J'.$row, "TOTAL RETUR PEMBELIAN:");
                            $sheet->getStyle('J'.$row)->getAlignment()->setHorizontal('center');
