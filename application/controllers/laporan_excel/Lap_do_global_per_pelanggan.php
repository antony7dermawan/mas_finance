<?php
      ...
      class Lap_do_global_per_pelanggan extends CI_Controller{
            ...
            public function index($date_from_laporan,$date_to_laporan,$type_id,$pelanggan_id)
            {
              ...
              $total_day=intval(((round(abs(strtotime($date_from_laporan) - strtotime($date_to_laporan)) / (60*60*24),0))+1)/2);
                ...
                $month_int = intval(date_format($date,"m"));
                  ...
                  $spreadsheet->getActiveSheet()->mergeCells('A'.$row.':J'.$row);
                  $sheet = $spreadsheet->getActiveSheet();
                  $sheet->setCellValue('A'.$row, 'Laporan DO GLOBAL per Pelanggan'.$pelanggan_id);
                  $sheet->getStyle('A'.$row)->getAlignment()->setHorizontal('center');
                  ...
                  $read_select = $this->m_t_t_t_penjualan_jasa_3->select_by_date_by_pelanggan($date_from_laporan,$date_to_laporan,1,$pelanggan_id);
