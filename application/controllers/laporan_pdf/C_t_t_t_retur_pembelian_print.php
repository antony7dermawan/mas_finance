...
class C_t_t_t_retur_pembelian_print extends MY_Controller
{
  ...
  public function index($retur_pembelian_id)
  {
    ...
    $read_select = $this->m_t_t_t_retur_pembelian_rincian->select($retur_pembelian_id);
    foreach ($read_select as $key => $value) 
    {
    ...
    for($i=0;$i<=$total_baris_transaksi;$i++)
    {
      ...
      if(($i>=$total_baris_1_bon and $rmd==0) or $i==0)
      {
        ...
        $pdf->Cell(30, 6, "RETUR PEMBELIAN", 0, 1, 'L');
