...
class c_faktur_penjualan_print extends MY_Controller
{
  ...
  public function index($id,$pks_id)
  {
    ...
    foreach ($read_select as $key => $value) 
    {
      ...
      if($key==0 or ($key>=$total_row_1_bon and $rmd==0))
      {
        ...
        if($keterangan_header!='')
        {
          ...
          $pdf->MultiCell($size[1], 8, substr($keterangan_header, 0, 200), 'L', 'L',0,0);
    ...
    { 
      ...
      $pdf->MultiCell($size[4], 8, '('.number_format(($value_diskon),2,'.',',').')' , 'LR', 'R',0,1);
        ...
        if($keterangan_footer!='')
        {
          $pdf->SetFont('','',9);
          $pdf->MultiCell($size[0], 8, '', 'L', 'C',0,0);
          $pdf->MultiCell($size[1], 8, substr($keterangan_footer, 0, 200), 'L', 'L',0,0);
    ...
    $total_pembayaran = $sum_total_tagihan + $sum_total_tagihan_ppn - $sum_value_pph ;
    $pdf->SetFont('','B',9);
    $pdf->Cell( $size[0]+$size[1],8,'','L',0,'R');
