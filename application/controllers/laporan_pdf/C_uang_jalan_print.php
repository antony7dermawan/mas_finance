  ...
  public function index($id)
  {
    ...
    foreach ($read_select as $key => $value) 
    {
      ...
      $r_tambahan=$value->TAMBAHAN;
        ...
        $pdf->Cell( 30,5,'Rp'.number_format($r_tambahan),'B',1,'R');


        $pdf->Cell( 15,5,'','0',0,'L');
        $pdf->Cell( 30,5,'b. Uang Operasional dijalan','B',0,'L');

        $pdf->Cell( 30,5,'Rp'.number_format($r_uang_jalan),'B',1,'R');


        $pdf->Cell( 15,5,'','0',0,'L');
        $pdf->Cell( 30,5,'','0',0,'L');

        $pdf->Cell( 30,5,'Rp'.number_format(($r_uang_jalan)+intval($r_tambahan)),'B',1,'R');
